...
if os.path.isfile(myLinks):
    ...
    forms.alert('Pouch is empty, nothing to remove.', title='Script cancelled')
    script.exit()
...
selectPaths = forms.SelectFromList.show(link_alias, 'Select items to remove', 500, 800, multiselect=True, button_name='Remove')
...
if selectPaths:
    ...
    try:
        ...
        forms.alert(str(rem_rows) + ' item(s) removed from pouch.', title='Script finished', warn_icon=False)
    # if the file could not be rewritten
    except:
        # display the outcome to the user
        forms.alert('Pouch is in unwritable state/location.', title='Script cancelled')
else:
    forms.alert('No items removed.', title='Script cancelled', warn_icon=False)