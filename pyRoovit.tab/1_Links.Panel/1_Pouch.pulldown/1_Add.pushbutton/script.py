...
# UI for link alias
sel_alias = forms.GetValueWindow.show('Set link alias', title='SET A NAME', width=500, height=600, default="")

# UI for link path
if sel_alias and sel_alias != "":
    sel_path = forms.GetValueWindow.show('Set link path', title='SET A PATH', width=500, height=600, default="")
else:
    forms.alert('No alias provided.', title='Script cancelled')
    script.exit()
...
if sel_path and sel_path != "":
    ...
    if os.path.isfile(myLinks):
        try:
            ...
            forms.alert('Links file in unreadable state/location.', title='Script cancelled')
            script.exit()
    #otherwise, try writing a new empty file
    else:
        try:
            with open(myLinks, "a") as file: 
                readLinks = "My Documents~" + userDocs
...
# if no path selected, cancel script     
    ...
    forms.alert('No link provided.', title='Script cancelled')
    script.exit()
...
if full_alias != None and full_alias not in readLinks:
    try:
        ...
        forms.alert('Link stored.', title='Script complete', warn_icon=False)
    except:
        forms.alert('Link was not able to be stored. Please contact the computational team to troubleshoot further.', title='Script cancelled')
